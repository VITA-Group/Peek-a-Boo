...
def main():
    ...
    print(f'Use batch size {args.batch_size}; learning rate scaled to {args.lr}')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.optimizer == 'SGD':
        ...
        bop_params = model_without_ddp.get_bop_params()
        non_bop_params = model_without_ddp.get_non_bop_params()
    ...
    if args.resume and args.resume.lower() != 'none':
        if os.path.isfile(args.resume):
            ...
            resume_msg = model_without_ddp.load_state_dict(checkpoint['state_dict'], strict=False)
            args.logger.info(str(resume_msg))
