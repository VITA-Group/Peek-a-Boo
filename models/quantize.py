...
class FPQuantizeFunction(InplaceFunction):
    ...
    def forward(ctx, input, num_bits=32, qparams=None, flatten_dims=_DEFAULT_FLATTEN,
                ...
                inplace=False, preserve_zero=True):
...
class FPQuantizeGradFunction(InplaceFunction):
    ...
    def forward(ctx, input, num_bits=32, qparams=None, flatten_dims=_DEFAULT_FLATTEN_GRAD,
                reduce_dim=0, dequantize=True, signed=True, stochastic=True, preserve_zero=True):
        ...
        ctx.inplace = False
        ctx.preserve_zero = preserve_zero
        return input
    ...
    def backward(ctx, grad_output):
        ...
        with torch.no_grad():
            ...
            grad_input = quantize(grad_output, num_bits=None,
                                  ...
                                  preserve_zero=ctx.preserve_zero)
        return grad_input, None, None, None, None, None, None, None, None, None


def quantize(x, num_bits=None, qparams=None,
             flatten_dims=_DEFAULT_FLATTEN, reduce_dim=0,
             dequantize=True, signed=False, stochastic=False,
             inplace=False, preserve_zero=True):
    return FPQuantizeFunction().apply(
        x, num_bits, qparams, flatten_dims, reduce_dim, dequantize, signed,
        stochastic, inplace, preserve_zero
    )
...
def quantize_grad(x, num_bits=None, qparams=None,
                  ...
                  dequantize=True, signed=True, stochastic=False, preserve_zero=True):
    return FPQuantizeGradFunction().apply(
        x, num_bits, qparams, flatten_dims, reduce_dim, dequantize, signed, stochastic, preserve_zero)
