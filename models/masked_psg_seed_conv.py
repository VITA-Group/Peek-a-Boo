...
class PredictiveSeedConv2d(Conv2d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 ...
                 # masked=False,  # if the network is masked; used for pruning
        ...
        super(PredictiveSeedConv2d, self).__init__(
        ...
        self.t = nn.Parameter(torch.rand(self.weight.shape, dtype=torch.float32) * 0.09 + 0.01)
    ...
    def forward(self, input):
        ...
        # Mask weight
        # weight = self.weight_mask * self.weight if self.masked else self.weight
        weight = self.weight_mask * self.weight * ste_sign(self.t)

        # Quantize weight
        weights = quant_weight(
            weight, num_bits_weight=self.num_bits_weight,
            ...
            counter=self.counter, preserve_zero=True
        )
        q_weight = weights[0]
